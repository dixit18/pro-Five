...
const Messages = () => {
  ...
  const tableActions = data?.map((item, i) => ({
    buyer: (
      <p
        className={`md:py-5 h-full text-xs font-semibold w-[90px] md:w-full md:text-sm truncate ${
          (authUser?.isServiceProvider && !item?.readBySeller) ||
          (!authUser.isServiceProvider && !item?.readByBuyer && "!bg-slate-100")
        }`}
      >
        {authUser?.isServiceProvider ? item.buyerId : item.sellerId}
      </p>
    ),
    lastMessage: (
      <Link
    ...
    action: (
      ...
      >
        {((authUser.isServiceProvider && !item.readBySeller) ||
          (!authUser.isServiceProvider && !item.readByBuyer)) && (
          <button
